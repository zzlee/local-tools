...
export function createProgram() {
  ...
  program
    ...
    .option("-p, --prompt <path>", "Specify a custom system prompt file", (value) => value, "prompts/default.md")
    .option("-s, --session <id>", "Resume a session with the given ID")
    .option("-f, --fork", "Pre-spawn the session with the system prompt")
    .option("-o, --output <path>", "Save the final cleaned output to a file")
    .option("-c, --continue", "Resume the last session or create a new one")
    .option("-k, --skill <skill>", "Enable a specific skill (can be used multiple times)", (value: string, prev: string[]) => {
      return prev ? [...prev, value] : [value];
    }, [] as string[])
    .option("--dry-run", "Show expanded prompt for custom commands without executing")
    .option("--list-sessions", "List all sessions")
    .option("--show-session <id>", "Show history of a specific session")
